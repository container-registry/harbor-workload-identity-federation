...
You need Go (the version in `go.mod`), [Task](https://taskfile.dev/installation/), and Helm. The extra CI tools are installed for you:

```bash
task setup
```

That installs `typos`, `lefthook`, `golangci-lint`, and `go-licenses`, then wires up the git hooks. The hooks spell check staged files and reject a commit whose subject is not conventional or that has no DCO sign-off. CI checks the pull request title rather than each commit, and the dco2 app checks sign-off, so the hooks catch locally what would otherwise come back as a failed check.
...
Run `task check` before pushing. It mirrors the spell check, commit lint, DCO, and license jobs.
