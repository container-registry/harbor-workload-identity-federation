...
| `securityContext` | `privileged: true` | The installer writes to the host filesystem and restarts kubelet. The schema refuses anything else, because relaxing it fails on the node rather than at install time. |
